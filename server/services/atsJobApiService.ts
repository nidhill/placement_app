import { JobListing, AtsJobFetchResult, AtsConnectionStatus, UserRole } from '../../src/types.ts';
import { dbStore } from '../db/store.ts';
import { JobNormalizer } from './jobNormalizer.ts';
import { TechJobClassifier } from './techClassifier.ts';
import { DesignationNormalizer } from './designationNormalizer.ts';
import { IndiaLocationFilter } from './indiaLocationFilter.ts';

/**
 * Standard Curated Technology Company Boards for Public ATS APIs.
 * These are completely public endpoints requiring no private credentials.
 * Can be overridden or extended via environment variables.
 */
...
export interface RawAtsJobItem {
  id: string | number;
  title: string;
  company: string;
  description: string;
  ...
  applyUrl: string;
  publishedAt: string;
  sourceProvider: 'Greenhouse' | 'Lever' | 'Ashby' | 'External API';
  employmentType?: string;
  salaryText?: string;
  rawPayload?: any;
}

export class AtsJobAdapter {
  /**
   * Transforms a raw ATS item into the canonical HACA JobListing schema.
   */
  public static normalizeRawJob(rawItem: RawAtsJobItem): {
    valid: boolean;
    isIndia: boolean;
    isTechJob: boolean;
    ...
    // 1. INDIA-ONLY HARD FILTER (Enforced before tech classification)
    ...
    const allSkills = JobNormalizer.extractSkills(title, description, classification.category);
    ...
    const { requirement: experienceRequirement, minYears: minExperienceYears } = JobNormalizer.parseExperience(undefined, description);
    ...
    // 6. Salary Mapping
    const salaryRange = rawItem.salaryText && rawItem.salaryText.trim()
      ? rawItem.salaryText.trim()
      : 'Competitive / Market Standard';

    // 7. Eligible Schools & Programs
    ...
    // Preserve official application URL for students
    ...
    const postedDate = rawItem.publishedAt || new Date().toISOString();
...
/**
 * Public ATS Job API Service
 * Queries Greenhouse, Lever, and Ashby public ATS APIs.
 * Supports configurable company targets via environment variables.
 */
export class AtsJobApiService {
  private static getGreenhouseBoards(): string[] {
  ...
  private static getLeverCompanies(): string[] {
  ...
  private static getAshbyCompanies(): string[] {
   ...
   */
  public static async testConnection(): Promise<AtsConnectionStatus> {
    ...
    try {
      if (ghBoards.length > 0) {
        const testRes = await fetch(`https://boards-api.greenhouse.io/v1/boards/${ghBoards[0]}/jobs`, { signal: AbortSignal.timeout(5000) });
        if (testRes.ok) {
      ...
      // greenhouse connection failed or timed out
    }

    // Test Lever
    try {
      if (leverCompanies.length > 0) {
        const testRes = await fetch(`https://api.lever.co/v0/postings/${leverCompanies[0]}?mode=json`, { signal: AbortSignal.timeout(5000) });
        if (testRes.ok) {
      ...
      // lever connection failed or timed out
    }

    // Test Ashby
    try {
      if (ashbyCompanies.length > 0) {
        const testRes = await fetch(`https://api.ashbyhq.com/posting-api/job-board/${ashbyCompanies[0]}`, { signal: AbortSignal.timeout(5000) });
        if (testRes.ok) {
      ...
      // ashby connection failed or timed out
    }
    ...
    const config = dbStore.getAtsConfig();
   ...
   */
  private static async fetchGreenhouseJobs(board: string): Promise<RawAtsJobItem[]> {
    const items: RawAtsJobItem[] = [];
    try {
      ...
      // Graceful error isolation per board
    }
    return items;
  }
   ...
   */
  private static async fetchLeverJobs(company: string): Promise<RawAtsJobItem[]> {
    const items: RawAtsJobItem[] = [];
    try {
      ...
      // Graceful error isolation per company
    }
    return items;
  }
   ...
   */
  private static async fetchAshbyJobs(company: string): Promise<RawAtsJobItem[]> {
    const items: RawAtsJobItem[] = [];
    try {
  ...
  public static async fetchAndIngestJobs(
  ...
  ): Promise<AtsJobFetchResult> {
    const limit = options?.limit || 50;
    const ghBoards = this.getGreenhouseBoards();
    const leverCompanies = this.getLeverCompanies();
    const ashbyCompanies = this.getAshbyCompanies();

    const rawItems: RawAtsJobItem[] = [];

    // Parallel ingestion across ATS platforms with error isolation
    const fetchPromises: Promise<RawAtsJobItem[]>[] = [];
    ...
    // 30-day dynamic freshness cutoff
    ...
    for (const raw of rawItems) {
      ...
      if (isNaN(postedDate.getTime()) || postedDate < thirtyDaysAgo) {
        // Job is older than 30 days or has an invalid date
        continue;
      }
      recentCount++;

      // 4. Tech Job Classification
      ...
      // Check if limit has been reached after processing valid results
      if (newIngested >= limit) {
        continue;
      }

      // 6. Ingest into database store
      const created = dbStore.createJob(normalized, actor);
      existingJobs.unshift(created); // Local cache deduplication
    ...
    // 7. Update ATS Stats in Store
    const atsConfig = dbStore.getAtsConfig();
    dbStore.updateAtsConfig({
      lastRunTimestamp: new Date().toISOString(),
      leadsProcessedTotal: atsConfig.leadsProcessedTotal + newIngested
    }, actor);

    // 8. Useful Server-Side Sync Logging (Exact format required by specification)
    console.log('----------------------------------------------------');
