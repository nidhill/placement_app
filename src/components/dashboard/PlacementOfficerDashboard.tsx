...
export const PlacementOfficerDashboard: React.FC<PlacementOfficerDashboardProps> = ({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error('Failed to load Placement Officer dashboard data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Exact 6 KPIs required for Placement Officer:
  // - Eligible Students
  // - Active Jobs
  // - Applications
  // - Interviews
  ...
  const pipelineStages = [
    { label: 'Applied', count: applications.filter(a => a.status === 'APPLIED').length, color: 'bg-slate-200 text-slate-700' },
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 bg-slate-900 text-white rounded-xl text-xs font-semibold hover:bg-slate-800 transition-colors shadow-2xs"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl text-xs font-semibold hover:bg-slate-50 transition-colors"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div 
          ...
          className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xs hover:border-slate-300 transition-colors cursor-pointer"
        >
          <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Eligible Students</div>
          <div className="mt-1 text-2xl font-bold text-slate-900">{eligibleStudents}</div>
        ...
        >
          ...
          <div className="mt-1 text-2xl font-bold text-slate-900">{scheduledInterviews}</div>
        ...
        >
          ...
          <div className="mt-1 text-2xl font-bold text-slate-900">{activeOffers}</div>
        ...
        >
          ...
          <div className="mt-1 text-2xl font-bold text-slate-900">{confirmedPlacements}</div>
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-slate-900">Active Application Pipeline</h3>
          <span className="text-xs text-slate-400">Total Active: {totalApplications} Candidates</span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-6 gap-2">
          {pipelineStages.map((stage, idx) => (
            <div key={stage.label} className="p-3 bg-slate-50 border border-slate-100 rounded-xl text-center">
              <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">
                {idx + 1}. {stage.label}
              </span>
              <div className="text-lg font-bold text-slate-900 mt-1">{stage.count}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-semibold text-slate-900">Students Requiring Follow-up</h3>
          ...
          <div className="pt-3 border-t border-slate-100">
            <button
              onClick={() => onNavigate('candidate_matching')}
              className="w-full py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 transition-colors text-center"
            >
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-semibold text-slate-900">Open Job Opportunities</h3>
              ...
              {recentActiveJobs.map(job => (
                <div key={job.id} className="py-2.5 flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => onNavigate('candidate_matching')}
                      className="px-2.5 py-1 bg-slate-900 text-white text-[10px] font-semibold rounded hover:bg-slate-800 transition-colors"
                    >
                      Match
                    </button>
                  </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs">
          ...
          <div className="space-y-3 mt-2 text-xs">
            {jobSources.map(source => (
              <div key={source.name} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
