    ...
    <div className="space-y-6">
      ...
      <div className="bg-primary text-white rounded-2xl p-6 shadow-md border border-primary/40">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="bg-primary/85/80 border border-slate-700 rounded-xl p-4 flex items-center gap-4">
      ...
      <div className="flex items-center gap-2 border-b border-border pb-2 overflow-x-auto text-sm">
        <button
          ...
          className={`px-3.5 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 whitespace-nowrap ${activeSubTab === 'users' ? 'bg-primary text-white' : 'text-slate-600 hover:bg-muted'}`}
        >
          <UserPlus className="w-4 h-4" /> User Provisioning & RBAC ({users.length})
        </button>
        <button
          onClick={() => setActiveSubTab('override')}
          className={`px-3.5 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 whitespace-nowrap ${activeSubTab === 'override' ? 'bg-primary text-white' : 'text-slate-600 hover:bg-muted'}`}
        >
          <ShieldAlert className="w-4 h-4 text-amber-400" /> Emergency Eligibility Override
        </button>
        <button
          onClick={() => setActiveSubTab('lms')}
          className={`px-3.5 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 whitespace-nowrap ${activeSubTab === 'lms' ? 'bg-primary text-white' : 'text-slate-600 hover:bg-muted'}`}
        >
          <RefreshCw className="w-4 h-4" /> Academic LMS Synchronization
        </button>
        <button
          onClick={() => setActiveSubTab('apify')}
          className={`px-3.5 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 whitespace-nowrap ${activeSubTab === 'apify' ? 'bg-primary text-white' : 'text-slate-600 hover:bg-muted'}`}
        >
          <Bot className="w-4 h-4" /> Apify Job Scraper Rules
        </button>
        <button
          onClick={() => setActiveSubTab('audit')}
          className={`px-3.5 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 whitespace-nowrap ${activeSubTab === 'audit' ? 'bg-primary text-white' : 'text-slate-600 hover:bg-muted'}`}
        >
      ...
      {activeSubTab === 'users' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-white p-5 rounded-2xl border border-border shadow-sm space-y-4">
            <div>
            ...
            <form onSubmit={handleProvisionUser} className="space-y-3.5">
              ...
              <button
                type="submit"
                className="w-full py-2.5 px-4 bg-primary hover:bg-primary/90 text-white font-medium text-xs rounded-lg transition-colors shadow-sm"
              >
                Provision Account
              </button>
            </form>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
            <div className="p-4 border-b border-border bg-muted flex items-center justify-between">
              <div>
                <h3 className="font-bold text-foreground text-sm">System Users & Accounts</h3>
      ...
      {activeSubTab === 'override' && (
        <div className="bg-white p-6 rounded-2xl border border-border shadow-sm max-w-3xl space-y-6">
          <div className="border-b border-border/70 pb-4">
            ...
            <h3 className="text-lg font-bold text-foreground">Emergency Student Eligibility Override</h3>
            <p className="text-xs text-slate-600 mt-1">
              The admin directly manages and overrides student eligibility states. All overrides are permanently recorded in the system audit log.
            </p>
          </div>
          ...
          <form onSubmit={handleExecuteEmergencyOverride} className="space-y-4">
            ...
            <div className="pt-2">
              <button
                type="submit"
                className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-medium text-xs rounded-lg transition-colors shadow-sm flex items-center gap-2"
              >
      ...
      {activeSubTab === 'lms' && (
        <div className="bg-white p-6 rounded-2xl border border-border shadow-sm space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/70 pb-4">
            <div>
              <h3 className="text-lg font-bold text-foreground">Academic LMS Synchronization Architecture</h3>
              <p className="text-xs text-slate-600 mt-0.5">
                Ingests student demographics, academic milestones, attendance records, and project evaluations.
              </p>
            </div>

            <div className="inline-flex items-center gap-2 px-3.5 py-2 bg-muted border border-border text-slate-700 text-xs font-semibold rounded-lg shadow-sm">
          ...
          <div className="p-4 rounded-lg bg-muted border border-border text-xs space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <code className="text-slate-800 bg-white px-2 py-1 rounded border border-border text-[11px] block mt-0.5">
      ...
      {activeSubTab === 'apify' && (
        <div className="bg-white p-6 rounded-2xl border border-border shadow-sm space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/70 pb-4">
            <div>
              <h3 className="text-lg font-bold text-foreground">AI Job Scraper Agent (Apify Integration)</h3>
              <p className="text-xs text-slate-600 mt-0.5">
            ...
            <div className="p-4 rounded-lg bg-muted border border-border">
              ...
              <div className="flex flex-wrap gap-1 mt-1.5">
                {apifyConfig?.targetBoards.map(b => (
                  <span key={b} className="text-[10px] px-2 py-0.5 rounded bg-white text-slate-700 border border-border font-medium">
                    {b}
                  </span>
                ))}
              </div>
      ...
      {/* 5. AUDIT LOGS TAB */}
      {activeSubTab === 'audit' && (
        <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden space-y-3">
          <div className="p-4 border-b border-border bg-muted flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h3 className="font-bold text-foreground text-sm">Immutable System Audit Trail</h3>
                ...
                {auditLogs
                  ...
                  .map(log => (
                    <tr key={log.id} className="hover:bg-muted transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className="px-2 py-0.5 rounded bg-muted text-slate-800 font-mono text-[10px] font-bold">
                          {log.action}
                        </span>
                      </td>
